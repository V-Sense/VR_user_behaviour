...
def main():
    ...
    def print_Results_opt(scenario_name, BitRate):
    ...
    ### coding rate r = R
    BitRate = [500, 760, 1005, 1529, 2326]#kbps
    ...
    lambda_vec = [0, 0.01, 0.05, 0.1, 0.25, 0.5, 1, 2]

                ###-------------------------------------------------###
                ###            Create saving file   (A)             ###
                ###-------------------------------------------------###
    ...
    for lambda_cost in lambda_vec:
        lambda_cost = float(lambda_cost)
        CostMax = lambda_cost
        
        print(name_file + '_' + videoName_all[0] + '_A_lambda_' +  str(lambda_cost))
        scenario =  directory + '/' + videoName_all[0] +'_A_lambda_' + str('%03f' % lambda_cost)
        ...
        for videoName in videoName_all:
            for ch in range(n_ch):
                ...
                alpha = pulp.LpVariable.dicts("Alpha_DecVar",(Users,Tiles,Rates,sp_index),0,1,pulp.LpInteger)
                beta = pulp.LpVariable.dicts('Beta_DecVar',(Tiles,Rates,sp_index),0,1,pulp.LpInteger)
#                gamma = pulp.LpVariable.dicts('Gamma_DecVar',(Users,sp_index),0,1,pulp.LpInteger)
                ...
                q = read_RD(videoName, ch)
        
                model += pulp.lpSum([q[int(u)][int(j)][int(r)][int(s)]*alpha[u][j][r][s]*S_tiles[int(j)]*prob[int(u)][int(j)]*Users_cat.Prob_u[int(u)] for u in Users for j in Tiles for r in Rates for s in sp_index] + [beta[j][r][s]*lambda_cost*(mu_s*BitRate[int(r)] + c_e(mu_e*length_ch, sp[int(s)][1])/N) for r in Rates for j in Tiles for s in sp_index]),"Obj fun"
                ...
                for u in Users:
                    for j in Tiles:
                        model += pulp.lpSum([alpha[u][j][r][s] for r in Rates for s in sp_index]) == 1, ""
                print("Constraint A - done")
                
                for s in sp_index:
                    for j in Tiles:
                        model += pulp.lpSum([beta[j][r][s] for r in Rates]) >= 1, ""
        ...
        #       ### --- b) set up decision variable alpha  and beta
                ...
                for u in Users:
                    model += pulp.lpSum([alpha[u][j][r][s]*BitRate[int(r)] for r in Rates for j in Tiles for s in sp_index]) <= Users_cat.BW_u[int(u)], ""
                    ...
                    for j in Tiles:
                        for s in sp_index:
                            if Devices[int(s)] == Users_cat.Device[int(u)]:
                                model += pulp.lpSum([alpha[u][j][r][s] for r in Rates]) == 1
                            else:
                                model += pulp.lpSum([alpha[u][j][r][s] for r in Rates]) == 0 


                        # The problem is solved using PuLP's choice of Solver
                model.solve()

                print_Results_opt('A', BitRate)
                
                del q, model, alpha, beta
                    
        print('Finished' + str(lambda_cost))
